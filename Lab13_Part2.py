...
def user_input(cmmd):
    if cmdExit.search(cmmd):                        # exits game if Player inputs "exit" or "quit"
        game_exit()
        # UPDATED: Added a return statement to handle clean game exit without JES error
        return False
      ...
      print p.location.description                # when Player inputs "look"
        ...
        # UPDATED: Following if statements added to assist with clean exit from game
        if examine == "chair":
          return False
        elif examine == "gyroscope":
          if itemTable["necklace"][1] is p and (itemTable["logic board"][1] is p or itemTable["tablet"][1] is p):
            return False
...
class Player:
        ...
        if item not in itemTable:
            ...
            if itemTable[item][3]:  # If true, places item in player inventory
                ...
                if item == "logic board" and self.location == secret_room:
                    ...
                    # UPDATED: Added flavor text when you have retrieved the logic board and teleported
                    showInformation("Do you think you're in control!? There is another! And they care not for your petty struggle!")
                if item == "book" and self.location == south_room:
                    print "You sense that your bag is heavier than before.\n"
                    itemTable["tablet"][1] = self
                # UPDATED: Added flavor text when retrieving the letter
                if item == "letter" and self.location == east_room:
                  showInformation("Did you really love her...?")
                # UPDATED: Added flavor text when retrieving the key
                if item == "key" and self.location == north_room:
                  showInformation("A key with no door. Like the lips of a lost love.")
            else:
    ...
    def examine_item(self, item):
      if item == "socket" and itemTable["key"][1] == self:
        ...
        # UPDATED: Added flavor text when player examines the tablet
        if item == "tablet":
          showInformation("Is it selfishness? Or stubborness? Either way, I'll enjoy the outsome. Hm hm...")
      else:
...
# UPDATED: Prints the welcome message as a text box, including the game commands
def print_welcome():
    """
    print "\t\t---------------------------------------------------------------\n"
...
# UPDATED: Prints an information box with flavor text
# Triggered when the player is in the west_room with items to complete the game
def last_plea(u,plea):
  if plea is True:
    return
  loc = p.location
  n = itemTable["necklace"][1]
  lb = itemTable["logic board"][1]
  t = itemTable["tablet"][1]
  if loc == west_room and n == p and lb == p and t != p:
    showInformation("Player!...Oh damn...what's your name...ah, here it is. %s! The book! The answer is in the book!" % u.upper())
    return True
  elif loc == west_room and n == p and t == p:
    showInformation("Oh %s. You think you're so clever...you cannot save her." % u)
    return True

# Drives the whole game.
def main():
    #done = False
    plea = False #UPDATED: Has last flavor text triggered yet
    print_welcome()
    #print_directions()
    # UPDATED: Player can now enter their real-life name which will be used in the game
    # for flavor text
    user_name = raw_input("The character has a name, but may we ask what your's is?: ")
    print ("\nThank you very much %s" % user_name)
    ...
    while True:
      ...
      # UPDATED: Check if game is ending and quit if so
      temp = user_input(raw_input(">>>", ))
      plea = last_plea(user_name,plea)
      if temp is False:
        break
      else:
        continue
